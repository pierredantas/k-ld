...
for a safety property that a faithful timer — K-ESBMC, corroborated by OpenPLC — proves
safe. K-ESBMC as the executable oracle exposes ESBMC's imprecise timer modelling.
...
|--------|--------|--------|-----------------|
| **K-ESBMC** (faithful) | **SAFE** | VIOLATED | `Light` trace `F,F,T,T,T,F,F` — on-delay PT=2, Q only while IN held |
| **ESBMC** v8.3.0 | **VIOLATED (false alarm)** | VIOLATED | counterexample has `Btn=0, T_Q=1, Light=1` — `Q` true while `IN` false |
| **OpenPLC/MATIEC** (ref) | holds | Light does turn on | validated scan-for-scan == K-ESBMC (see `../validation/`) |
...
- K-ESBMC, being an executable IEC-faithful semantics (validated against OpenPLC), is a
...
# K-ESBMC: drive Btn held (T,T,T,T,T,F,F), read Light from <trace> -> F,F,T,T,T,F,F
...
sees the light turn on. K-ESBMC (faithful TOF, validated vs OpenPLC) exposes the
...
into K-ESBMC DSL and reproduces the P1 violation **identically** to the hand-translation
...
## Result (K-ESBMC trace; buttons unpressed, PIR pulse F,T,F,F,F,F)
...
- **K-ESBMC:** P1 VIOLATED @scan 2 (light on while corridor empty, no button state).
...
- **OpenPLC/MATIEC:** the TOF holds Q for PT after IN drops (validated == K-ESBMC in
  `../validation/`), so the light stays on after the PIR clears — corroborating K-ESBMC.
...
and `traffic_light_unsafe` (identical programs): **K-ESBMC SAFE, ESBMC UNSAFE**. Triage:
...
- K-ESBMC run with `Enable` held for 30 scans: **0 mutual-exclusion violations**. In fact
  ...
  but no *safety* property is ever violated. K-ESBMC SAFE is correct, and the argument is
  preset-independent.

So all three disagreements in the full table resolve in K-ESBMC's favour, exhibiting both
of ESBMC's opposite timer-handling failures:

| Program | ESBMC | K-ESBMC | ESBMC failure |
|---------|-------|------|---------------|
| stairs_light | SAFE | UNSAFE(P1) | **skip** timer → missed bug (unsound) |
| traffic_light (≡ _unsafe) | UNSAFE | SAFE | **havoc** timer → false alarm |

## Bottom line
The K-ESBMC oracle (validated scan-for-scan against OpenPLC) is correct on every
